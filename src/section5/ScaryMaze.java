...
public class ScaryMaze extends JPanel implements Runnable, MouseMotionListener {
	...
	public void mouseMoved(MouseEvent e) {
		...
		int endColor = -11927788;
		
		//8. If the mouse falls off the path (if it is on the background)
		if (mouseColor == backgroundColor) {
				// call the scare method
			scare();
		} else if (mouseColor == endColor) { //13. If the mouse is on the end color
			// pop up a message to tell them they won
			System.out.println("At the end!");
			JOptionPane.showMessageDialog(null, "YOU WON! You finished the maze!");
		} else {
			System.out.println("Safe on the path!");
		}
